...
export async function getNotificationCounts(): Promise<NotificationCounts> {
  ...
  const supabase = await createClient();
  const [inquiries, viewingRequests] = await Promise.all([
...
export async function getAnalyticsOverview(): Promise<AnalyticsOverview> {
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    dailyViews.push({ date: key, count: countsByDay.get(key) ?? 0 });
  }
  ...
  return {
    ...
    viewsLast30DaysTrend: computeTrend(last30.length, prev30.length),
